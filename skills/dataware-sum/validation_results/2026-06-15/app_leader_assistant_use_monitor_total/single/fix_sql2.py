...
for tc in test_cases:
    ...
    if 'DERIVED' in tc['name']:
        ...
        # 构建子查询中的字段列表
        if 'REGION_MANAGE' in tc['name']:
            sub_select = 'dt, rpt_date, sum_type, first_tab_type, region_code,'
        else:
            sub_select = 'dt, rpt_date, sum_type, first_tab_type,'
...
ON {join_cond}
WHERE ABS(exp.exp_value - COALESCE(act.{field_name}, 0)) > 0.01 OR act.{field_name} IS NULL
LIMIT 10;"""
...
with open('test_cases_fixed2.json', 'w', encoding='utf-8') as f:
