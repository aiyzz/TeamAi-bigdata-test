# dwd.waybill_sign_waybill_track_dd

- **表名**：dwd.waybill_sign_waybill_track_dd
- **层级**：DWD
- **主键**：waybill_no
- **分区字段**：dt
- **数据来源**：D:\yto\wiki\dataware\tables\dwd.waybill_sign_waybill_track_dd.md
...
| order_channel_code | string | 订单渠道 | 维度 |
...
| start_depart_truck_no1_bra | string | 始发第一个分公司发车车签 | 维度 |
...
| start_depart_truck_no1 | string | 经转顺序第一个中心发车车签 | 维度 |
...
| etl_create_time | string | 创建时间 | 维度 |
| etl_modify_time | string | 修改时间 | 维度 |

## 业务说明

- 本表为内部轨迹表（签收口径），将包裹流转节点"拉平"展示
- 用于全链路时效分析和路由复盘
