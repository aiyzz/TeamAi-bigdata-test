# dwd.waybill_sign_waybill_info_dd

- **表名**：dwd.waybill_sign_waybill_info_dd
- **层级**：DWD
- **主键**：waybill_no
- **分区字段**：dt
- **数据来源**：D:\yto\wiki\dataware\tables\dwd.waybill_sign_waybill_info_dd.md
...
| receiver_province_name | string | 收件省份 | 维度 |
| receiver_city_name | string | 收件城市 | 维度 |
| receiver_area_name | string | 收件区域 | 维度 |
| receiver_town_name | string | 收件乡镇 | 维度 |
...
| end_org_code | string | 目的网点 | 维度（关联键） |
| signature_delivery_fail_reason | string | 派送失败原因描述 | 维度 |
| call_delivery_tag | string | 外呼结果 | 维度 |

### 质检、风控与客服意向

| 字段名 | 类型 | 注释 | 分类 |
|--------|------|------|------|
| risk_level | string | 风险级别 | 维度 |
| risk_code | string | 风险原因编码 | 维度 |
| signoff_distance | string | 签收距离 | 指标 |
| verification_type | string | 质检证明 | 维度 |
| verify_status | string | 质检状态 | 维度 |
| customer_intention | string | 客户意向 | 维度 |
| answered | string | 95161是否打通 | 维度 |
| subscribe_site_code | string | 物料申购网点编码 | 维度 |
...
- 本表为签收口径运单信息表，已签收的运单才会出现在此表
- 无需通过 op_code 维表筛选，本表本身就是签收场景
