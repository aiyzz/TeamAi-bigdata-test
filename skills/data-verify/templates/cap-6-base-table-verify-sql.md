...
WHERE NVL(CAST(NULLIF(t.<测试字段>, '') AS STRING), 'XXT')
    <> NVL(CAST(NULLIF(b.<基准字段>, '') AS STRING), 'XXT')
LIMIT 100;
```
...
SELECT
  ...
  SUM(
    CASE
      WHEN NVL(CAST(NULLIF(t.<测试字段>, '') AS STRING), 'XXT')
         = NVL(CAST(NULLIF(b.<基准字段>, '') AS STRING), 'XXT')
      THEN 1 ELSE 0
    END
  ) AS match_cnt,
...
WHERE NVL(CAST(NULLIF(t.<测试字段>, '') AS STRING), 'XXT')
    <> NVL(CAST(NULLIF(dim_op.oper_type, '') AS STRING), 'XXT')
LIMIT 100;
```
...
WHERE NVL(CAST(NULLIF(t.<测试字段>, '') AS STRING), 'XXT')
    <> NVL(CAST(NULLIF(dim_org.<维表字段>, '') AS STRING), 'XXT')
LIMIT 100;
```
...
WHERE NVL(CAST(NULLIF(t.<测试字段>, '') AS STRING), 'XXT')
    <> NVL(CAST(NULLIF(<转换表达式>, '') AS STRING), 'XXT')
LIMIT 100;
```
