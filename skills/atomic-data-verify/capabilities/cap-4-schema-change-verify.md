...
对共同字段（排除主键和分区字段），按 `field-compare-sql.md` 批量比对：
...
SELECT
  '<新增字段>' AS field_name,
  SUM(CASE WHEN t.<新增字段> != src.<来源字段>
       OR (t.<新增字段> IS NULL AND src.<来源字段> IS NOT NULL)
       OR (t.<新增字段> IS NOT NULL AND src.<来源字段> IS NULL)
       THEN 1 ELSE 0 END) AS diff_cnt
FROM <测试表> t
...
**D3 — ETL 转换**：按 `etl-transform-sql.md` 逐字段验证。
