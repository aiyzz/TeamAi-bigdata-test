...
- **迁移来源**：dataware_table/nike.ods_jsc_t_market_direct_customer_dd.md
- **最后更新**：迁移自动生成
...
| billing_node | varchar(255) | 运费计费节点 SIGN-签收 COLLECT-揽收 CENTER_COLLECT-中心下车（揽收） | 维度 |
...
| settle_code | varchar(255) | 结算编码 | 维度（筛选条件） |
| settle_name | varchar(255) | 结算名称 | 维度 |
| file_paths | text | 附件路径 json 字符串 | 维度 |
...
| status | varchar(255) | 审核状态 0 待审核 1 审核通过 2 驳回 3作废 | 维度 |
...
- **关联键**：k_code（对应测试表的 kcode）

## 筛选条件

- **筛选条件**：settle_code LIKE 'ZK%'
